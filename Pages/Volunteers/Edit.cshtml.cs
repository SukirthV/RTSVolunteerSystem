    ...
    public class EditModel : PageModel
    {
        ...
        public void OnPost()
        {
            ...
            if (volunteerInfo.id.Length == 0 || volunteerInfo.fullname.Length == 0 || volunteerInfo.email.Length == 0 ||
                volunteerInfo.password.Length == 0 || volunteerInfo.phone.Length == 0 || volunteerInfo.address.Length == 0 ||
                volunteerInfo.ecname.Length == 0 || volunteerInfo.ecphone.Length == 0 || volunteerInfo.dob.Length == 0 ||
                volunteerInfo.availablefrom.Length == 0 || volunteerInfo.availableto.Length == 0 || volunteerInfo.comments.Length == 0)
            {
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    String sql = "UPDATE volunteers " +
                                 "SET fullname=@fullname, email=@email, password=@password, phone=@phone, address=@address, ecname=@ecname, ecphone=@ecphone, " +
                                 "dob=@dob, availablefrom=@availablefrom, availableto=@availableto, comments=@comments " +
                                 "WHERE id=@id";
                    using (SqlCommand command = new SqlCommand(sql, connection))
                    {
